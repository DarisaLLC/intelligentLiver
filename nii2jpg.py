...
def save_nii_roi_png(nii_file, roi_nii_files, output_root):
    ...
    for roi_nii_file in roi_nii_files:
        ...
        if np_roi.shape != np_image_shape:
            print(np_roi.shape, np_image_shape, nii_file)
            continue
        np_roi_xyz = np.transpose(np.nonzero(np_roi))
        print(np_roi_xyz.shape)
...
nii_root = 'F:\\huaxi\\Liver-EOB-NII'
png_root = 'F:\\huaxi\\Liver-EOB-JPG'
for sub_root, sub_dirs, files in os.walk(nii_root):
    ...
    roi_HBP_nii_files = glob(os.path.join(sub_root, sub_name + '_roi*_HBP.nii.gz'))
